...
import 'package:bsudoku/components/popups.dart';
...
class GameScreenProvider with ChangeNotifier {
    ...
    Future<void> onSettingsPressed() async {
        ...
        await GameRoutes.goTo(GameRoutes.optionsScreen, enableBack: true, callBackAfter: _resumeGame);
        notifyListeners();
    }
    ...
    void _createNewGame(GameModel newGame) {
        ...
        if (gameModel.isOnGoingGame) {
            debugPrint('Continuing ${gameModel.difficulty.name.toUpperCase()} game - TIME: ${gameModel.time.toTimeString()}');
        } else {
            _clearGame();
            debugPrint('Creating a new ${gameModel.difficulty.name.toUpperCase()} game ');
            _fillTheBoard();
            _saveGameStarted();
        }
    ...
    Future<void> _saveGameStarted() async {
        ...
        GameStatsModel gameStatsModel = GameStatsModel(difficulty: difficulty, dateTime: now, startTime: now);
    ...
    void _fillNumbers() {
        ...
        for (var y = 0; y < 9; y++) {
            ...
            for (var x = 0; x < 9; x++) {
                ...
                final Set<int> intersectedValues = sudokuBoard.getIntersectedValues(cell);

                if (intersectedValues.length <= 8) {
                    final int randomNumber = generateRandomNumber(exclude: intersectedValues);
    ...
    void _giveStartNumbers() {
        for (final CellPositionModel cellPosition in getRandomPositions(difficulty)) {
    ...
    void pauseButtonOnTap() {
        Popup.gamePaused( time: time, mistakes: mistakes, difficulty: difficulty, onResume: _resumeGame);
    ...
    void _selectCell(CellModel cellModel) => selectedCell = cellModel;
    ...
    void _highlightCells(CellModel cellModel) {
        ...
        sudokuBoard.allCells.where((element) => element.position.x == cellModel.position.x ||
    ...
    void _removeAllHighlights() {
        sudokuBoard.allCells.where((element) => element.isHighlighted).forEach((element) {
            element.isHighlighted = false;
            sudokuBoard.updateCell(element);
        });
    }

    void addMoveToLog({ required int value, required int oldValue, required List<int> notes, required List<int> oldNotes }) {
        final MoveModel moveModel = MoveModel( cellPosition: selectedCell.position, value: value, oldValue: oldValue, notes: notes, oldNotes: oldNotes );
    ...
    void _enterNote(int number) {
        ...
        addMoveToLog(value: 0, oldValue: selectedCell.value, notes: List<int>.from(selectedCell.notes), oldNotes: oldNotes);
        _clearValue();
    }
    ...
    void _enterValue(int number, {bool isHint = false}) {
        if (selectedCell.value != number) {
            if (isHint) {
                ...
                addMoveToLog(value: number, oldValue: selectedCell.value, notes: [], oldNotes: List<int>.from(selectedCell.notes));
            }
            _clearNotes();
    ...
    Future<void> _saveGameStats() async {
        ...
        GameStatsModel gameStatsModel = GameStatsModel(dateTime: now, startTime: now, difficulty: difficulty, won: false, score: score, time: time, mistakes: mistakes);
    ...
    Future<void> _chooseNewGameDifficulty() async {
        ...
        try {
            chosenDifficulty = await ModalBottomSheets.chooseDifficulty(restartDifficulty: difficulty);
        } catch (e) {
          debugPrint(e.toString());
        }

        if (chosenDifficulty != null) {
            final GameModel gameModel = GameModel(sudokuBoard: GameSettings.createSudokuBoard(), difficulty: chosenDifficulty);
            _createNewGame(gameModel);
        } else {
            Future.delayed(const Duration(milliseconds: 300), () => Popup.gameOver(onNewGame: _chooseNewGameDifficulty, onExit: _onExit));
        }
    }
    ...
    void _deleteLastNote() {
        ...
        addMoveToLog(value: 0, oldValue: 0, notes: List<int>.from(selectedCell.notes), oldNotes: oldNotes);
    ...
    void _deleteNumber() {
        if (selectedCell.hasValue) {
            addMoveToLog(value: 0, oldValue: selectedCell.value, notes: [], oldNotes: [] );
            selectedCell.value = 0;
            _updateSelectedCell();
        }
    }
    ...
    bool get _canGetHint => !selectedCell.isGivenNumber && !selectedCell.isValueCorrect && hints > 0;
    ...
    bool isNumberButtonNecessary(int number) {
        return sudokuBoard.allCells.where((element) => element.value == number).toList().length < 9;
    }
